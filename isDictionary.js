...
const isDictionary = (A, B) => {
  // create a map of alphabets and order count in hashmap for give alien language (B)
  let alienLang = new Map();

  for (let i = 0; i < 26; i++) {
    alienLang.set(B[i], i + 1);
  }

  console.log('sl', alienLang);

  for (let word of A) {
    for (let j = 0; j < word.length - 1; j++) {
      let first = alienLang.get(word[j]);
      console.log('hee', first, word);
      if (alienLang.get(word[j + 1]) >= first) continue;
      else return 0;
    }
  }
